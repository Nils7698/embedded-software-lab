...
int main(int argc, const char* argv[]) {
    ...
    auto start_cgc = std::chrono::high_resolution_clock::now();
    createGoldCodes(goldCodes);
    auto end_cgc = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration_cgc = end_cgc - start_cgc;
    ...
    std::chrono::duration<double> duration_is = end_is - start_is;
    ...
    std::cout << "Laufzeit des Signal-Lesens: " << duration_gs.count() << " Sekunden / " << duration_gs.count() / duration.count() * 100 << "%"<< std::endl;
    std::cout << "Laufzeit der Gold-Code-Erstellung: " << duration_cgc.count() << " Sekunden / " << duration_cgc.count() / duration.count() * 100 << "%" << std::endl;
    std::cout << "Laufzeit der Signalinterpretation: " << duration_is.count() << " Sekunden / "  << duration_is.count() / duration.count() *100 << "%" << std::endl;
    std::cout << "Laufzeit des gesamten Programms: " << duration.count() << " Sekunden" << std::endl;

    return 0;
    //Command für Compilierung: g++ -O2 -o decode_chrono decode_chrono.cpp
}
